...
\textit{rating} (Calificaciones) y la \textit{vote\_mean} esten
...
\begin{figure}[h!]
	...
	\caption{Diagrama~\cite{biplot}\textit{Biplot}. Este diagrama representa los valores de
		...
		Estos colores corresponden a tres segmentos de calificaciones: $>2$, entre $2$ y $3.5$ y $>4$.
		También se pueden apreciar los vectores correspondientes a las variables originales.}
	\label{fig:biplot}
\end{figure}
...
mejor idea de cual es su valor medio y la dispersion que se puede esperar.
...
\section{Algoritmo de los K vencinos cercanos (\textit{K-Nearest-Neighbor} o \textit{KNN})}

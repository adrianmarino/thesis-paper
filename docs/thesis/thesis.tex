...
c se describe la distribución de valores de la métrica \textit{mAP@5(4,5)}
evaluada en el conjunto de observaciones de validación. Se pueden apreciar por
valores atípicos a ambos extremos y un leve sesgo a izquierda, debido al aporte
del modelos basado en items.
...
En la figura~\ref{fig:nnFmMAP} se pueden observar valores atípicos en ambos
extremos. Fuera de esto la distribución tiene no presenta sesgos a simple
vista. Por otro lado, pretans dos modas en la aprte central y el valir medio
aprece tener un valor similar a una de las dos modas. El valor medio de la
precisión es de $0.393447$ con una dispersión de $0.011711$. La media es muy
cercana a la mediana, con una diferencia de $5*10^{-5}$.
...
En la figura~\ref{fig:deepFmLoss} se observan el cruce entre las curvas de
validación y entrenamiento en la segunda iteración, punto en el cual el modelo
generaliza mejor según esta metricas. Luego, se puede apreciar que el modelo
comienza a sobre ajusta inmediatamente hasta estabilizarse con una diferencia
de casi una unidad de error (\textit{RMSE}).

En cuando a la métricas \textit{mAP@5(4,5)}, en la figura~\ref{fig:deepFmMAP}
se observa que el modelo tiene un valor medio de $0.397895$ y una dispersión
$0.011369$ quedando por debajo de los modelos \textit{GMF} ($0.408787$) y
\textit{B-GMF} ($0.406646$) respectivamente.
...
En la figura~\ref{fig:deepFmRMSE} se observan que el error (\textit{RMSE}) de

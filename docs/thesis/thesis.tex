...
\textit{feature} definido para los items. Con esta información, es posible
...
interacciones de usuarios con items (Películas en este caso). Además, dado que
...
variables originales. Luego, las cargas o \textit{loadings} son los coeficientes
...
que las variables que mas aportan a un campo en la misma son \textit{User ID 
(99\%)} y \textit{Release Year (45\%)} respectivamente, ambos positivos.
...
modelo tomo como entrada un usuario (\textit{user\_id}) y un item (\textit{item\_id}) y retorna la
...
otro lado, es una evolución del modelo \textit{Wide and Deep}~\cite{wideanddeeppaper} de \textit{Google}, el cual es un ensample de dos
...
evitando el sobre ajuste(\textit{overfitting}) para aquellos modelos que tienden a sobre

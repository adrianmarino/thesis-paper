...
A continuación se puede ver un gráfico que describe las clasificaciones y sub-clasificaciones de los sistemas de recomendación:
...
Para realizar este trabajo se selecciono el dominio del cine, ya que existen conjuntos de datos bien definidor y actualizados. Por otro lado, es el dominio clásico para papers y literatura de sistemas de recomendación en general.

Dada la propuesta de este trabajo, es necesario contar con datos de interacciones de usuario con items(películas). Ademas, dado que lo buscamos solucionar el problema de cold-start con el enfoque de filtros colaborativos, se necesitara contar otro enfoque de recomendación, el cual posiblemente pueda ser uno basado en contenido. Por esta cuestión, necesitamos contar con features completos y consistentes para los items(películas).

Dadas estas necesidades se decidió utilizar los siguientes datasets:

\subsection{MovieLens 25M Dataset}

Esta dataset prácticamente no tiene información de lis items(películas) pero si tiene las calificaciones de realizadas por los usuarios. También contamos con un conjunto de tags o palabras clase cargadas por los usuarios para cada item(película). Otro punto importante, es que todos los usuarios tienen al menos 20 interacciones, lo cual asegura que no habrá problemas de baja performance por falta de datos. De esta forma, este dataset sera muy util para entrenar a modelos de recomendación basados en filtros colaborativos y ademas cuenta con columnas extras como tags que serán útiles a la hora de entrenar modelos basados en contenido.


\subsection{TMDB Movie Dataset}

Este dataset no tiene calificaciones personalizadas de los items como si sucede con el dataset anterior, pero tiene varios features de los items que pueden ser muy útiles para modelos basados, en contenido en inclusive modelos híbridos, los cuales buscamos explorar. Contamos con features de tipos variados:

\begin{itemize}
	\item Título.
	\item Lenguaje nativo: Lenguaje original en el cual fue filmada la película.
	\item Overview: Sinopsis de la película.
	\item Géneros: Ambos dataset cuentan con una lista de géneros a los que adiete la película.
	\item Enlaces al detalle de la película en imdb y the movie database. Estos enlaces permiten hace join con mas datos que se encuentren en la Descripción de estos sitios pero en este trabajo solo se utilizara la imagen pata de la película a modo de visualización.
	\item  Fecha de lanzamiento.
	\item Si es una película para adultos.
	\item Presupuesto destinado para realizar el film.
	\item Popularidad.
...
Dada la necesitada de utilizar datos de ambos datasets se realizo un merge o join con ambas fuentes de datos. Ya que ambos datasets cuentan con la columna imdb\_id fue la elegida para realizar el merge. Particularmente se realiza un merge de los csv o tablas que representan a los items(películas). Finamente contamso con dos tablas principales. Una que contiene las interacciones usuario item:

\begin{itemize}
	\item user_id
	\item movie_id
	\item timestamp: Fecha en la que se realizó la calificación del item.
	\item rating: Calificación. Es un valor discreto numérico: 

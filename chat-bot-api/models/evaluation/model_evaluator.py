    ...
    def run(self):
        ...
        for user_id in user_ids:
            ...
            while True:
                ...
                result_relevant_items = list(filter(lambda x: int(x.id) in rating_by_remaining_item_id.keys(), result.items))
                ...
                if len(result_relevant_items) <= 1:
                    ...
                    patience += 1
                    if self.verbose:
                        logging.info(f'{log_prefix}Not found relevant items into list of {len(result.items)} recommendations - Patience: {patience}/{max_patience}.')
                    continue
                else:
                    patience = 0
                ...
                if self.verbose:
                    logging.info(f'{log_prefix}Found {len(result_relevant_items)} relevant items from a list of {len(result.items)} - Votes: {", ".join(votes)}.')

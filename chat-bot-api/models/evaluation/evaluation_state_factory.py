...
class EvaluationStateFactory:
    ...
    def create(
        ...
        max_patience={40: 2, 100: 3, 200: 4, 300: 5},
        plot_interval=10,
        verbose=False,
    ):
        if not verbose:
    ...
    @classmethod
    def _reset_env(cls, api_client, interactions_test_set, items):
    ...
    def _genres_count(cls, interactions_test_set, items, user_id):
        ...
        for item_id in item_ids:
            genres.extend(
                items[items["movie_id"] == item_id]["movie_genres"].tolist()[0]
            )
    ...
    def releases(cls, interactions_test_set, items, user_id):
        ...
        return np.sort(
            items[items["movie_id"].isin(df["item_id"].unique())][
                "movie_release_year"
            ].unique()
        )
    ...
    def _create_fake_profiles(cls, interactions_test_set, items):
        ...
        for user_id in user_ids:
            ...
            genres_count_df = cls._genres_count(
                interactions_test_set, items, user_id=user_id
            )
            genres = genres_count_df[genres_count_df["count"] >= 10]["index"].tolist()

            release = str(
                cls.releases(interactions_test_set, items, user_id=user_id)[0]
            )

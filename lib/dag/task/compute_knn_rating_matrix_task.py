...
def python_callable(
    ...
    def load_df(path_key):
        ...
        return pd.read_json(f'{domain.cfg.temp_path}/{path_value}', orient='records')

    def train_predict(train_df, test_df, columns, model_name):
        model_type = ml.KNNType.USER_BASED if 'knn_user_based' == model_name else ml.KNNType.ITEM_BASED
    ...
    future_interactions, filtered_train_interactions = domain.interaction_inference_service.predict(
        ...
        train_predict_fn=lambda train, test, cols: train_predict(train, test, cols, model),

...
class RecSysDataset(Dataset):
    def __init__(self, dataset, transform, target_transform, device = torch.device("cpu")):
    ...
    @property
    def features(self):
        return self._to_feats(self.data)

    @property
    def targets(self):
        return self._to_target(self.data)
    ...
    @property
    def target_uniques(self):
        return self.targets_value_counts[0]

    @property
    def features_value_counts(self):
        features = self.features
        return np.array(
            [np.unique(features[:, col_idx].cpu().numpy(), return_counts=True) for col_idx in range(features.shape[1])],
            dtype=object)
